...
-- 관리자가 회원의 미승인된 입고 요청 내역이 있는 회원의 아이디를 조회하는 프로시저
-- -> 선택한 회원의 입고요청상세 전체 내역 조회 : procedure show_all_inbound_request_items;
drop procedure if exists get_all_member_has_approved_inbound_requests;
delimiter @@
create procedure get_all_member_has_approved_inbound_requests()
begin
    -- 회원 ID, 미승인 건수 출력
    select member_id, count(member_id) as approved_request_num
    from inbound_request
    where inbound_request_approved = 1
    group by member_id
    order by count(member_id) desc;
end @@
delimiter ;

select * from inbound_request where member_id = 'usera002';

call get_all_member_has_approved_inbound_requests();
...
create procedure get_unapproved_inbounds_by_member(
	IN c_member_id varchar(15))
begin
	select I.member_id, I.coffee_id, C.coffee_name, I.inbound_request_id, I.inbound_request_item_id, I.inbound_request_quantity, I.inbound_request_date
    from inbound_request_items I
    join inbound_request R on I.member_id = R.member_id and I.inbound_request_id = R.inbound_request_id
...
select I.member_id, C.coffee_name, I.inbound_request_id, I.inbound_request_item_id, I.inbound_request_quantity, I.inbound_request_date
from inbound_request_items I
         join inbound_request R on I.member_id = R.member_id and I.inbound_request_id = R.inbound_request_id
         join coffees C on C.coffee_id = I.coffee_id
where R.inbound_request_approved = 0 AND I.member_id = 'usera003'
order by I.inbound_request_date;

-- 회원의 입고 요청 중 회원 아이디 및 입고 요청 아이디로 조회하는 프로시저
drop procedure if exists get_one_inbound_by_member_requestId;
delimiter @@
create procedure get_one_inbound_by_member_requestId(
    IN c_member_id varchar(15),
    IN c_inbound_request_id char(12))
begin
    select I.member_id, I.coffee_id, C.coffee_name, I.inbound_request_id, I.inbound_request_item_id, I.inbound_request_quantity, I.inbound_request_date
    from inbound_request_items I
             join inbound_request R on I.member_id = R.member_id and I.inbound_request_id = R.inbound_request_id
             join coffees C on C.coffee_id = I.coffee_id
    where I.inbound_request_id = c_inbound_request_id AND I.member_id = c_member_id
    order by I.inbound_request_date;
end @@
delimiter ;

call get_one_inbound_by_member_requestId('usera001','inbound00001');
...
create procedure get_approved_inbounds_by_member(
	IN c_member_id varchar(15))
begin
	select I.member_id, C.coffee_id, C.coffee_name, I.inbound_request_id, I.inbound_request_item_id, I.inbound_request_quantity, I.inbound_request_date
    from inbound_request_items I
    join inbound_request R on I.member_id = R.member_id and I.inbound_request_id = R.inbound_request_id
    join coffees C on C.coffee_id = I.coffee_id
...
DROP PROCEDURE IF EXISTS update_member_inbound_request;
-- 입고 요청 및 상세 내역을 트랜잭션으로 처리하는 프로시저
-- c_inbound_request_id: 입고 요청 고유 ID
-- c_member_id: 요청을 하는 회원 ID
-- c_manager_id: 관리자 ID (두 테이블 모두에 사용)
...
-- c_inbound_request_date: 회원이 요청한 입고 날짜
DELIMITER @@
CREATE PROCEDURE update_member_inbound_request(
    IN c_inbound_request_id CHAR(12),
    IN c_member_id VARCHAR(15),
    IN c_manager_id VARCHAR(15),
    IN c_request_items_json TEXT,
...
BEGIN
    ...
    -- 2. 기존 입고 요청 상세 항목 (inbound_request_items) 데이터 삭제
    -- 'inbound_request_id'에 해당하는 모든 기존 데이터를 지웁니다.
    DELETE FROM inbound_request_items
    WHERE inbound_request_id = c_inbound_request_id AND member_id = c_member_id;

    -- 3. 새로운 입고 요청 상세 항목 (inbound_request_items) 데이터 삽입
    -- JSON 배열을 파싱하여 새로운 아이템들을 삽입합니다.
    INSERT INTO inbound_request_items (
        inbound_request_id,
        member_id,
        manager_id,
...
--  ---------------------------------------
-- update_member_inbound_request 테스트
--  --------------------------------------
select * from inbound_request_items where member_id = 'usera003';
set @test_inbound_request_id = 'inbound00003';
set @test_member_id = 'usera003';
set @test_manager_id = 'manager9104';
set @test_inbound_request_date = CURDATE();
SET @test_items_json = (
    SELECT JSON_ARRAYAGG(
                   JSON_OBJECT('coffee_id', coffee_id, 'quantity', FLOOR(10 + (RAND() * 90)))
           )
...
call update_member_inbound_request(
     @test_inbound_request_id, @test_member_id, @test_manager_id, @test_items_json, @test_inbound_request_date, @result
     );
...
select @inbound_request_id_to_process;
...
update inbound_request set inbound_request_approved = 0 where inbound_request_id =@inbound_request_id_to_process;
...
begin
    ...
    join inbound_request R on I.inbound_request_id = R.inbound_request_id and I.member_id = R.inbound_request_id
...
-- 1.4 회원의 입고요청 하나 조회
drop procedure if exists show_inbound_request_items;
delimiter @@
create procedure show_inbound_request_items(IN c_member_id varchar(15), IN c_inbound_request_id char(12))
begin
	select C.coffee_name, C.coffee_id,I.inbound_request_id, I.inbound_request_quantity, I.inbound_request_date, I.inbound_request_item_id
	from inbound_request_items I
    join coffees C on I.coffee_id = C.coffee_id
    join inbound_request R on I.inbound_request_id = R.inbound_request_id and I.member_id = R.member_id
    where I.member_id = c_member_id AND I.inbound_request_id = c_inbound_request_id;
end @@
delimiter ;

call show_inbound_request_items('usera001', 'inbound00001');
...
CALL show_inbound_request_items(@member_id_one_item, @inbound_request_id_one_item);
...
begin
	select I.member_id, I.inbound_request_id, C.coffee_name, C.coffee_id, I.inbound_request_quantity, I.inbound_request_date, inbound_request_item_id
    from inbound_request_items I
    join coffees C on I.coffee_id = C.coffee_id
    join inbound_request R on I.inbound_request_id = R.inbound_request_id and I.member_id = R.member_id
