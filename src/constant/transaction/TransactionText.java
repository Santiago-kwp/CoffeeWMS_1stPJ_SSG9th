...
public enum TransactionText {
  ...
  MEMBER_INBOUND_LIST_HEADER("-".repeat(55)+"<< 입고 요청 내역 조회 (회원) >>" + "-".repeat(55)),
  MEMBER_INBOUND_LIST("1. 입고 요청 수정 | 2. 입고 요청 취소 | 3. 뒤로가기"),
  MEMBER_INBOUND_COMPLETION_LIST_HEADER("-".repeat(55)+"<< 입고 완료 현황 조회 (회원) >>" + "-".repeat(55)),
  MEMBER_INBOUND_COMPLETION_LIST("1. 기간별 입고 현황 | 2. 월별 입고 현황 | 3. 뒤로가기"),

  INBOUND_COFFEE_SELECT("입고할 커피를 번호로 선택하세요."),
  INBOUND_COFFEE_QUANTITY_INPUT("입고 요청 커피 수량 (발주 가능 수량은 최대 100개)"),
  ...
  MANAGER_INBOUND_LIST("1. 회원 입고 요청 승인 | 2. 입고 요청 수정 | 3. 입고 요청 취소 | 3. 뒤로가가"),
  ...
  UNAPPROVED_INBOUNDS_LIST("-".repeat(50)+"<< 미승인된 입고요청 내역 >>" + "-".repeat(50)),
  UNAPPROVED_INBOUND_LIST_HEADER(String.format("%-15s| %-15s\t | %-10s | %-15s\n", "회원 ID", "커피 이름", "수량", "입고 요청 날짜")),
